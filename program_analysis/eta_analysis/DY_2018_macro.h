...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   public:
    ...
    bool smearing = true;
    double smear_value1, smear_value2;
    ...
    TString saving_path = "/afs/cern.ch/user/n/nrawal/work/DY_analysis/program_analysis/eta_analysis/plots_smear_2nd_tmp/";
    ...
    std::pair<double,double> mean_sigma_positive_reco_eta[8];
    std::pair<double,double> mean_sigma_negative_reco_eta[8];
    ...
    double mean_positive_reco_eta[8];
    double sigma_positive_reco_eta[8];
 
    double mean_negative_reco_eta[8];
    double sigma_negative_reco_eta[8];
 

    double diff_Zmass_positive_gen_reco[8];
    double diff_Zmass_negative_gen_reco[8];
    ...
    // new declaration of histograms
     ...
     TH1D *histogram_Zmass_positive_eta_smear[8];
     TH1D *histogram_Zmass_negative_eta_smear[8];
    ...
    void graph_mean_eta_reco();
    void graph_mean_eta();
    void graph_diff_Zmass_gen_reco();
    void smearing_pT();
    void reconstruct_Zmass();

    void histogram_declaration_eta_smear();
    void histogram_declaration_eta();

    void histogram_filling_positive_eta_smear(int i);
    void histogram_filling_negative_eta_smear(int i);
    void histogram_filling_positive_eta(int i);
    void histogram_filling_negative_eta(int i);
};

   // pT  smered
  void DY_2018_macro :: histogram_declaration_eta_smear(){
   for(int i=0; i<8;i++)
   {
    TString histogram_name_positive = TString::Format("histogram_Zmass_positive_eta_smear[%d]",i);
   TString title_positive = TString::Format("Gen Z mass after smearing for  #mu^{+} : %.1f  #leq #eta < %.1f" ,eta_list[i], eta_list[i+1]) ; 
   histogram_Zmass_positive_eta_smear[i] = new TH1D(histogram_name_positive,title_positive, 300,60,120);
   }
   for(int i=0;i<8;i++){
    TString histogram_name_negative = TString::Format("histogram_Zmass_negative_eta_smear[%d]",i);
    TString title_negative = TString::Format("Gen Z mass after smearing for  #mu^{-} : %.1f  #leq #eta < %.1f" ,eta_list[i], eta_list[i+1]) ; 
    histogram_Zmass_negative_eta_smear[i] = new TH1D(histogram_name_negative,title_negative, 300,60,120);
   }
  }


  void DY_2018_macro :: histogram_filling_positive_eta_smear(int i){
   histogram_Zmass_positive_eta_smear[i]->Fill(massZ_reconstruct); 
  } 

  void DY_2018_macro :: histogram_filling_negative_eta_smear(int i){
   histogram_Zmass_negative_eta_smear[i]->Fill(massZ_reconstruct); 
  } 
 ...
 void DY_2018_macro :: graph_mean_eta_reco(){
   TGraphErrors *gr_positive_reco = new TGraphErrors(8, eta_list_mean,mean_positive_reco_eta, 0,sigma_positive_reco_eta);
   gr_positive_reco->GetYaxis()->SetTitle("mass_{(#mu^{+}#mu^{-})}");
   gr_positive_reco->GetXaxis()->SetTitle("#eta");
   gr_positive_reco->SetTitle("#mu^{+}");
   gr_positive_reco->GetYaxis()->SetRangeUser(91.1,91.25);
   TCanvas *graph_canvas = new TCanvas("graph_canvas","mass(#mu^{+}#mu^{-})",900,600);
   graph_canvas->cd(); 
   
   gr_positive_reco->Draw("AP*");
   graph_canvas->SaveAs(saving_path+"mean_eta_mu_positive_reco.pdf"); 
 
   TGraphErrors *gr_negative_reco = new TGraphErrors(8, eta_list_mean,mean_negative_reco_eta, 0,sigma_negative_reco_eta);
   gr_negative_reco->GetYaxis()->SetTitle("mass_{(#mu^{+}#mu^{-})}");
   gr_negative_reco->GetXaxis()->SetTitle("#eta");
   gr_negative_reco->SetTitle("#mu^{-}");
   gr_negative_reco->GetYaxis()->SetRangeUser(91.1,91.25);
   TCanvas *graph_canvas_1 = new TCanvas("graph_canvas_1","mass(#mu^{+}#mu^{-})",900,600);
   graph_canvas_1->cd(); 
   
   gr_negative_reco->Draw("AP*");
   graph_canvas_1->SaveAs(saving_path+"mean_eta_mu_negative_reco.pdf"); 
  
   }

 void DY_2018_macro :: graph_diff_Zmass_gen_reco(){
   TGraph *gr_diff_gen_reco_positive = new TGraph(8, eta_list_mean,diff_Zmass_positive_gen_reco);
   gr_diff_gen_reco_positive->GetYaxis()->SetTitle("mass_{(#mu^{+}#mu^{-})}");
   gr_diff_gen_reco_positive->GetXaxis()->SetTitle("#eta");
   ...
   TGraph *gr_diff_gen_reco_negative = new TGraph(8, eta_list_mean,diff_Zmass_negative_gen_reco);
   gr_diff_gen_reco_negative->GetYaxis()->SetTitle("mass_{(#mu^{+}#mu^{-})}");
   gr_diff_gen_reco_negative->GetXaxis()->SetTitle("#eta");
   gr_diff_gen_reco_negative->SetTitle("#mu^{-}");
   TCanvas *graph_canvas_1 = new TCanvas("graph_canvas_1","diff Gen and Reco Z mass",900,600);
   graph_canvas_1->cd(); 
   ...
   gr_positive->GetYaxis()->SetRangeUser(90.8,91.6);
   TCanvas *graph_canvas = new TCanvas("graph_canvas","mass(#mu^{+}#mu^{-})",900,600);
   graph_canvas->cd(); 
   ...
   gr_negative->GetYaxis()->SetRangeUser(90.8,91.6);
   TCanvas *graph_canvas_1 = new TCanvas("graph_canvas_1","mass(#mu^{+}#mu^{-})",900,600);
   graph_canvas_1->cd(); 
...
smear_value1 = random->Gaus(0,0.015*genLep_pt1);
smear_value2 = random->Gaus(0,0.015*genLep_pt2);
